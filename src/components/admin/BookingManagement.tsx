...
const BookingManagement = () => {
  ...
  const updateStatus = async (
    ...
    if (error) {
      ...
      await supabase.functions.invoke("send-booking-email", {
        body: { type: status, booking: { ...booking, ...updates } },
      });
      fetchBookings();
    }
  };
  ...
  const saveNotes = async (id: string) => {
    const { error } = await supabase
      .from("bookings")
      .update({ admin_notes: noteInput[id] || "" })
      .eq("id", id);
    if (error) {
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <span
                      className={`px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider rounded ${statusColors[booking.status] || ""}`}
                    >
                      {booking.status}
                    </span>
                  </div>
              ...
              {expandedId === booking.id && (
                <div className="px-5 pb-5 border-t border-border pt-4 space-y-4">
                  ...
                  {booking.project_description && (
                    <div>
                      <p className="font-body text-xs text-muted-foreground uppercase tracking-[0.15em] mb-1">
                        Project Details
                      </p>
                      <p className="font-body text-sm bg-muted/50 p-3">
                        {booking.project_description}
                      </p>
                    </div>
                  )}
